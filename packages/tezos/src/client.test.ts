...
import { createCubidTezosClient } from "./client";
import {
  getCubidTezosCapabilities,
  hasCubidTezosCapability
} from "./internal";

describe("@cubid/tezos", () => {
  it("keeps Tezos adapter logic behind the adapter boundary and only persists through @cubid/core", async () => {
    const apiClient = {
    ...
    const adapter = {
      connect: vi.fn(async () => ({
        address: "tz1TezosExampleWalletAddress1111111111111",
        capabilities: {
          sessionKeys: {
            available: false,
        ...
        chainType: "tezos",
        networkId: "ghostnet",
        publicKey: "edpktetezospublickey111111111111111111111"
      })),
      id: "tezos-wallet",
      verify: vi.fn(async () => ({
        isVerified: true,
        metadata: { signature: "tezos-signature" }
      }))
    };

    const client = createCubidTezosClient(apiClient as never);
    ...
    expect(apiClient.addStamp).toHaveBeenCalledWith({
      ...
      stampData: {
        ...
        chainType: "tezos",
        curve: "ed25519",
        identity: "tz1TezosExampleWalletAddress1111111111111",
        networkId: "ghostnet",
        publicKey: "edpktetezospublickey111111111111111111111",
        uniquevalue: "tz1TezosExampleWalletAddress1111111111111",
        verification: { signature: "tezos-signature" },
        walletType: "tezos"
      },
      stampType: "tezos",
      userId: "user-1"
    });
    expect(getCubidTezosCapabilities(connection)).toEqual({
      sessionKeys: {
        available: false,
        metadata: { reason: "not_enabled" }
      }
    });
    expect(hasCubidTezosCapability(connection, "sessionKeys")).toBe(false);
  ...
  it("throws when stamp persistence is requested without enough context", async () => {
    ...
    await expect(
      client.verifyWallet({
        adapter: {
          connect: async () => ({ address: "tezos1examplewalletaddress000000000000000" }),
          id: "adapter",
          verify
        },
        connection: { address: "tz1TezosExampleWalletAddress1111111111111" },
        persistStamp: true
      })
  ...
  it("throws when stamp persistence is requested without adapter verification", async () => {
    ...
    await expect(
      client.verifyWallet({
        adapter: {
          connect: async () => ({ address: "tz1TezosExampleWalletAddress1111111111111" }),
          id: "adapter"
        },
        connection: { address: "tz1TezosExampleWalletAddress1111111111111" },
        pageId: 4,
  ...
  it("uses the default unverified shape when the adapter does not verify", async () => {
    ...
    expect(result.connection.address).toBe("tz1TezosExampleWalletAddress1111111111111");
