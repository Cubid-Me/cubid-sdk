import fs from "node:fs";

import { describe, expect, it, vi } from "vitest";

import { createCubidPolkadotClient } from "./client";
import {
  getCubidPolkadotCapabilities,
  hasCubidPolkadotCapability
} from "./internal";

describe("@cubid/polkadot", () => {
  it("keeps Polkadot adapter logic behind the adapter boundary and only persists through @cubid/core", async () => {
    const apiClient = {
      addStamp: vi.fn(async () => ({ success: true })),
      config: {
        apiKey: "api-key",
        baseUrl: "https://passport.cubid.me/api/v2",
        dappId: "dapp-id",
        fetch
      }
    } as const;

    const adapter = {
      connect: vi.fn(async () => ({
        address: "12ExamplePolkadotWalletAddress11111111111111",
        capabilities: {
          sessionKeys: {
            available: false,
            metadata: { reason: "not_enabled" }
          }
        },
        chainType: "polkadot",
        genesisHash: "0x91b171bb158e2d3848fa23a9f1c25182",
        publicKey: "0x03polkadotpublickey",
        ss58Format: 0
      })),
      id: "polkadot-wallet",
      verify: vi.fn(async () => ({
        isVerified: true,
        metadata: { signature: "polkadot-signature" }
      }))
    };

    const client = createCubidPolkadotClient(apiClient as never);
    const connection = await client.connectWallet({ adapter });
    const result = await client.verifyWallet({
      adapter,
      connection,
      pageId: 4,
      persistStamp: true,
      stampType: "polkadot",
      userId: "user-1"
    });

    expect(adapter.verify).toHaveBeenCalledWith(
      expect.objectContaining({
        address: "12ExamplePolkadotWalletAddress11111111111111",
        chainType: "polkadot",
        genesisHash: "0x91b171bb158e2d3848fa23a9f1c25182",
        publicKey: "0x03polkadotpublickey",
        ss58Format: 0
      })
    );
    expect(apiClient.addStamp).toHaveBeenCalledWith({
      pageId: 4,
      stampData: {
        address: "12ExamplePolkadotWalletAddress11111111111111",
        chainType: "polkadot",
        genesisHash: "0x91b171bb158e2d3848fa23a9f1c25182",
        identity: "12ExamplePolkadotWalletAddress11111111111111",
        publicKey: "0x03polkadotpublickey",
        ss58Format: 0,
        uniquevalue: "12ExamplePolkadotWalletAddress11111111111111",
        verification: { signature: "polkadot-signature" },
        walletType: "polkadot"
      },
      stampType: "polkadot",
      userId: "user-1"
    });
    expect(getCubidPolkadotCapabilities(connection)).toEqual({
      sessionKeys: {
        available: false,
        metadata: { reason: "not_enabled" }
      }
    });
    expect(hasCubidPolkadotCapability(connection, "sessionKeys")).toBe(false);
    expect(result.capabilities).toEqual({
      sessionKeys: {
        available: false,
        metadata: { reason: "not_enabled" }
      }
    });
    expect(result.verification.isVerified).toBe(true);
    expect(result.persistedStamp?.success).toBe(true);
  });

  it("throws when stamp persistence is requested without enough context", async () => {
    const apiClient = {
      addStamp: vi.fn(async () => ({ success: true })),
      config: {
        apiKey: "api-key",
        baseUrl: "https://passport.cubid.me/api/v2",
        dappId: "dapp-id",
        fetch
      }
    } as const;

    const client = createCubidPolkadotClient(apiClient as never);
    const verify = vi.fn(async () => ({ isVerified: true }));

    await expect(
      client.verifyWallet({
        adapter: {
          connect: async () => ({ address: "12ExamplePolkadotWalletAddress11111111111111" }),
          id: "adapter",
          verify
        },
        connection: { address: "12ExamplePolkadotWalletAddress11111111111111" },
        persistStamp: true
      })
    ).rejects.toThrow("Persisting a wallet stamp requires both userId and pageId.");
    expect(verify).not.toHaveBeenCalled();
  });

  it("throws when stamp persistence is requested without adapter verification", async () => {
    const apiClient = {
      addStamp: vi.fn(async () => ({ success: true })),
      config: {
        apiKey: "api-key",
        baseUrl: "https://passport.cubid.me/api/v2",
        dappId: "dapp-id",
        fetch
      }
    } as const;

    const client = createCubidPolkadotClient(apiClient as never);

    await expect(
      client.verifyWallet({
        adapter: {
          connect: async () => ({ address: "12ExamplePolkadotWalletAddress11111111111111" }),
          id: "adapter"
        },
        connection: { address: "12ExamplePolkadotWalletAddress11111111111111" },
        pageId: 4,
        persistStamp: true,
        userId: "user-1"
      })
    ).rejects.toThrow("Persisting a wallet stamp requires an adapter.verify implementation.");
  });

  it("uses the default unverified shape when the adapter does not verify", async () => {
    const apiClient = {
      addStamp: vi.fn(async () => ({ success: true })),
      config: {
        apiKey: "api-key",
        baseUrl: "https://passport.cubid.me/api/v2",
        dappId: "dapp-id",
        fetch
      }
    } as const;

    const client = createCubidPolkadotClient(apiClient as never);

    const result = await client.verifyWallet({
      adapter: {
        connect: async () => ({ address: "12ExamplePolkadotWalletAddress11111111111111" }),
        id: "adapter"
      },
      connection: { address: "12ExamplePolkadotWalletAddress11111111111111" },
      stampType: "polkadot"
    });

    expect(result.connection.address).toBe("12ExamplePolkadotWalletAddress11111111111111");
    expect(result.verification).toEqual({ isVerified: false });
    expect(result.persistedStamp).toBeUndefined();
    expect(result.stampData).toEqual({
      address: "12ExamplePolkadotWalletAddress11111111111111",
      chainType: undefined,
      genesisHash: undefined,
      identity: "12ExamplePolkadotWalletAddress11111111111111",
      publicKey: undefined,
      ss58Format: undefined,
      uniquevalue: "12ExamplePolkadotWalletAddress11111111111111",
      verification: undefined,
      walletType: "polkadot"
    });
  });

  it("does not declare any web2 dependency edge", () => {
    const packageJson = JSON.parse(
      fs.readFileSync(new URL("../package.json", import.meta.url), "utf8")
    ) as {
      dependencies?: Record<string, string>;
    };

    expect(packageJson.dependencies?.["@cubid/web2"]).toBeUndefined();
    expect(packageJson.dependencies?.["@cubid/web2-react"]).toBeUndefined();
  });
});
