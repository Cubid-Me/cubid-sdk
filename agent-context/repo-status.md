...
| Active roadmap closeout | `Partial` Remaining non-closed roadmap items are `S03` and `S04` in `agent-context/todo.md`; `S04.1` is now complete and the next auth follow-up is `S04.2`. |
