...
| MyPayTag paytag realignment | `Pass` S18/S19 replace the old Pay-To public SDK surface with Paytag-only helpers and close MVP gaps: typed hosted Paytag actions, opaque-by-default alias creation, OpenAPI coverage for all implemented Paytag routes, browser hosted-action hardening, Paytag/OpenAPI drift guardrails, no payment-intent helper in the paytag surface, and no list-all-paytags resolver helper. |
