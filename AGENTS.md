...
- `packages/bitcoin`, `packages/cosmos`, `packages/near`, `packages/polkadot`, `packages/solana`, `packages/starknet`, and `packages/sui` now carry additional chain-specific wallet layers on top of `@cubid/core`.
